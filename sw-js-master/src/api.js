...
export class User {
  static fetchProfile(slug, perPage) {
    return getWithCredentials(`${API_URL}/users/${slug}?perPage=${perPage}`);
  }
  ...
  static logout() {
    return fetch(`${API_URL}/users/sign_out`, {
      method: 'DELETE',
      credentials: 'include'
    })
  }
...
export class Publisher {
  static fetchProfile(slug, perPage) {
    return getWithCredentials(
      `${API_URL}/publishers/${slug}?perPage=${perPage}`
    );
  }
...
export class Book {
  ...
  static relevel(slug, newLevel) {
    const payload = {
      ...
      commit: 'Relevel Story',
      story_id: slug
    };

    return postWithCredentials(`${API_URL}/stories/${slug}/relevels`, JSON.stringify(payload));
  }
