  ...
  renderActionBar(baseClassName) {
    ...
    return (
      <ActionBar
        ...
        <ActionBarSection>
          <Stat
            value={readingList.likeCount}
            icon={readingList.liked ? 'heart' : 'heart-outline'}
            iconVariant="accent"
            label={readingList.likeCount ? null : t("global.like", 1)}
            onClick={readingList.liked ? null : () => this.props.onLikeButtonClicked(readingList.slug)}
            />
          ...
          <List inline={true}>
            {
              ...
              <OfflineActions
                ...
                onClickAddToOffline={() => {
                  const bookCountAfterSave = offlineBooks.books.length + readingList.books.filter(book => isAvailableOffline(book.id, offlineBooks)).length
