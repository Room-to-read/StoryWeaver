...
import { links, delays, userFeedbackPopups, gaEventCategories, gaEventActions } from '../../lib/constants';
...
const mapStateToProps = ({ readingLists, user, viewport, offlineBooks }) => ({
  ...
  onLikeButtonClicked = () => {
    ...
    if (!isLoggedIn) {
      return addNotification({
        type: 'info',
        id: 'ReadingList.anonymous-like-notification',
        dedup: true,
        title: t('ReadingList.anonymous-like-notification-title'),
        content: t('ReadingList.anonymous-like-notification-text'),
        additionalActions: [{
  ...
  render() {
    ...
    const onClickAddToOffline = () => {
      const saveBooksPromise = books
        ...
        .map(saveOfflineWorkflow)

      return Promise.all(saveBooksPromise)
    }
    const onClickRemoveFromOffline = () => books.map(book => unsaveOffline(book.id))
