...
import {
  ...
  fetchReadingListsWorkflow,
} from '../../redux/readingListsSearchActions';
...
const mapStateToProps = ({ listsSearch, viewport, allFilters: { listsFilters } }) => ({
  ...listsSearch,
  viewport,
...
class ListSearchResultsContainer extends Component {
  ...
  onLoadMore = () => this.props.fetchReadingListsWorkflow(
  ...
  componentWillReceiveProps(nextProps) {
    if (!isEqual(this.props.appliedFilters, nextProps.appliedFilters)) {
      this.props.fetchReadingListsWorkflow(nextProps.appliedFilters);
    }
  }
  ...
  componentDidMount() {
    ...
    fetchReadingListsFiltersWorkflow().then(
      () => fetchReadingListsWorkflow(appliedFilters)
    );
  }
  ...
  render() {
    ...
    if (viewport.medium) {
      filtersComponent = (
        <FiltersBar
          ...
          itemTypeLabel={t('global.list', totalListsCount)}
    ...
    const shouldShowLoadMore = lists && lists.length !== 0 ? (lists.length < totalListsCount ? true : false) : false;
    
    return (
      <div>
        { filtersComponent }
        {
          isFetchingLists || !lists
          ?
          <LoaderBlock />
          :
          <div>
            ...
            />
            {
              ...
              <Pagination
                ...
                loading={this.props.isFetchingMoreLists}
              />
              :
              null
            }
