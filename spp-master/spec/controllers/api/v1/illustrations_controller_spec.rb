...
#SHOW
  context "SHOW/GET Illustrations Details" do
    it "should show the illustration details" do
      ...
      expect_status(200)
      expect(ok: true)
      expect(JSON.parse(response.body)['data'].keys).to contain_exactly('title', 'id', 'slug', 'categories', 'styles', 'license_type', 'attribution_text', 'liked', 'likesCount', 'readsCount', 'isFlagged', 'tags', 'imageUrls', 'illustrators', 'similarillustrations', 'downloadLinks', 'publisher', 'usedIn')
