...
describe Api::V1::HomeController, :type => :controller do
  ...
  context "GET current user details" do
    it "should show current user details" do
      ...
      get :me, format: :json
      expect_json(ok: true)
      expect(JSON.parse(response.body)['data'].keys).to contain_exactly("id", "email", "first_name", "last_name", "role", "roles", "organizationRoles", "isOrganisationMember", "hasHistory", "locale", "isLoggedIn", "bookshelf", "homePopup", "offlineBookPopupSeen", "popupsSeen", "languagePreferences", "readingLevels")
