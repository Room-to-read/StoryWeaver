class ContestsController < ApplicationController
  ...
  def contest_details
    ...
    if @contest.name.downcase.match("spotathon")
      @current_tab = params[:filter].present? ? params[:filter] : "easy"
      if params[:filter] == "over_view"
        spotathon_contest_overview
      elsif params[:filter] == "medium"
        if params[:reviewed] == "reviewed"
          @story = Story.includes(:ratings, :youngsters).reorder("ratings.user_id DESC").where("ratings.user_id = ?", current_user.id).where(:contest => @contest, :status => Story.statuses[:published], :reading_level => Story.reading_levels["2"]).page(params[:page]).per(12)
        else
          @story = Story.includes(:ratings, :youngsters).reorder("ratings.user_id DESC").where("ratings.user_id IS ? OR ratings.user_id != ?", nil, current_user.id).where(:contest => @contest, :status => Story.statuses[:published], :reading_level => Story.reading_levels["2"]).page(params[:page]).per(12)
          @rating = Rating.new
        end
      else
        if params[:reviewed] == "reviewed"
          @story = Story.includes(:ratings, :youngsters).reorder("ratings.user_id DESC").where("ratings.user_id = ?", current_user.id).where(:contest => @contest, :status => Story.statuses[:published], :reading_level => Story.reading_levels["1"]).page(params[:page]).per(12)
        else
          @story = Story.includes(:ratings, :youngsters).reorder("ratings.user_id DESC").where("ratings.user_id IS ? OR ratings.user_id != ?", nil, current_user.id).where(:contest => @contest, :status => Story.statuses[:published], :reading_level => Story.reading_levels["1"]).page(params[:page]).per(12)
          @rating = Rating.new
        end
      end
    else
  ...
  def spotathon_contest_overview
    get_stories(Story.statuses[:published])
      @languages = @stories.collect(&:language).uniq
      @languages.each {|language| @all[language.name] = {}}
      @story_types = ["easy", "medium", "total"]
      @all["Total"] = {}
      @stories.each do |story|
        if story.reading_level == "1"
          get_statistics(story, @story_types[0])
        elsif story.reading_level == "2"
          get_statistics(story, @story_types[1])
        end
        get_statistics(story, @story_types[2])
      end
  end

  def rrr_contest_overview
    get_stories(Story.statuses[:submitted])
    @languages = @contest.languages
    @languages.each {|language| @all[language.name] = {}}
    @story_types = ["adult", "total"]
    @all["Total"] = {}
    @stories.each do |story|
      if !story.youngsters.present?
        get_statistics(story, @story_types[0])
      #else
        #get_statistics(story, @story_types[1])
      end
