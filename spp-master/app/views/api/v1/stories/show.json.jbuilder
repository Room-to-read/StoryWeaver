...
json.data do
  ...
  json.downloadLinks ["Low resolution pdf", "A4 size (Print ready pdf)", "epub"].each_with_index do |type|
    json.type type 
    json.href ((type =="Low resolution pdf") ||(current_user && current_user.content_manager?)) ? get_download_link(@story, type) : nil 
  end
